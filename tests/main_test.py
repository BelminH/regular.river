...
from main import get_file_name, \
    get_transactions, classify_transactions, delete_files, \
    update_sheet, return_credentials, process_file
...
class TestGetFileName(unittest.TestCase):
    ...
    def test_get_file_name_success(self, mock_print, mock_input, mock_exists, mock_isfile, mock_access,
                                   mock_is_valid_csv, mock_rename_csv):
...
@pytest.mark.parametrize("transactions,expected_totals,expected_unknown", [
    (
            [("REMA 1000", 100), ("BUNNPRIS", 50), ("NETFLIX", 15), ("Komplett", 120), ("Unknown Merchant", 25)],
    ...
    (
            [("Unknown Merchant 1", 25), ("Unknown Merchant 2", 35), ("Unknown Merchant 3", 45)],
            {"Food and Groceries": 0, "Entertainment": 0, "Electronic": 0},
            [("Unknown Merchant 1", 25), ("Unknown Merchant 2", 35), ("Unknown Merchant 3", 45)]
    ),
    (
...
def test_classify_transactions(default_dicts, transactions, expected_totals, expected_unknown):
    categories, totals, skip = default_dicts
...
def mock_datetime():
    with patch('main.datetime') as mock_datetime:
        mock_datetime.now.return_value.month = 3  # Mocking as if the current month is March
        yield mock_datetime
...
def test_delete_files():
    ...
    with patch('main.os') as mock_os:
        ...
        expected_calls = [mock_os.path.join(test_dir, file_name) for file_name in test_files]
        actual_calls = [call_args[0][0] for call_args in mock_os.remove.call_args_list]
        assert actual_calls == expected_calls, "os.remove was not called with the expected file paths"
        assert mock_os.remove.call_count == len(
            test_files), f"os.remove was expected to be called {len(test_files)} times, but was called {mock_os.remove.call_count} times"
...
def test_return_credentials():
    ...
    with patch("main.open", mock_open(read_data=json.dumps(mock_data))) as mock_file:
        with patch("main.json.load", return_value=mock_data) as mock_json:
            ...
            assert spreadsheet_id == "test_spreadsheet_id", f"Expected spreadsheet_id to be 'test_spreadsheet_id', got '{spreadsheet_id}'"
            assert sheet_name == "test_sheet_name", f"Expected sheet_name to be 'test_sheet_name', got '{sheet_name}'"
            assert sheet_id == "test_sheet_id", f"Expected sheet_id to be 'test_sheet_id', got '{sheet_id}'"